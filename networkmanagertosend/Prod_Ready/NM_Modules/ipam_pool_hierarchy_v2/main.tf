locals {
    ...
    hub_mask = var.regional_ipam_pool_hierarchy.hub_pool.subnet_mask
    spokes_mask = var.regional_ipam_pool_hierarchy.spokes_pool.subnet_mask
...
module "hub_ipam_pool" {
    ...
    address_prefixes = [local.hub_cidr]
...
module "spokes_ipam_pools" {
    ...
    # Use the exact subnet mask from the config
    address_prefixes = [local.spokes_cidr]
...
module "monitoring_ipam_pool" {
    ...
    # Use the exact subnet mask from the config
    address_prefixes = [local.monitoring_cidr]
